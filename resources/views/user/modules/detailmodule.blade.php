        ...
        <div class="flex-1 p-8 bg-black">
            ...
            <div class="mb-8">
                ...
                <div class="w-full bg-gray-700 rounded-full h-2.5">
                    <div class="bg-green-500 h-2.5 rounded-full" style="width: {{ $module->progress_for_current_user ?? 0 }}%"></div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="md:col-span-1 bg-gray-900 border border-gray-700 rounded-lg p-4">
                    ...
                    <ul class="space-y-2">
                            ...
                            <li>
                                <a href="{{ route('detail.points', ['module' => $module->id, 'point' => $p->id]) }}"
                                   class="flex items-center gap-2 px-3 py-2 rounded-lg transition-colors
                                    ...
                                    {{-- SVG --}}
                                    <svg class="w-5 h-5 {{ $isCompleted ? 'text-green-500' : 'text-gray-500' }}" fill="currentColor" viewBox="0 0 24 24">
                                        @if($isCompleted)
                                            <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.532-1.676-1.676a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" clip-rule="evenodd" />
                                        @else
                                            <path d="M5 3.5a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1H5.5a.5.5 0 0 1-.5-.5Zm0 3a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1H5.5a.5.5 0 0 1-.5-.5Zm0 3a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1H5.5a.5.5 0 0 1-.5-.5ZM3 10a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1h7a1 1 0 0 1 1 1v4a1 1 0 0 1-1 1H3Zm0 1a1 1 0 0 0-1 1v4a1 1 0 0 0 1 1h7a1 1 0 0 0 1-1v-4a1 1 0 0 0-1-1H3Zm0 1a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1H3.5a.5.5 0 0 1-.5-.5Z"/>
                                        @endif
                                    </svg>
                        ...
                        @empty
                            <li><p class="text-gray-400">Tidak ada poin untuk modul ini.</p></li>
                        @endforelse
                    </ul>
                </div>
                ...
                <div class="md:col-span-3 bg-gray-900 border border-gray-700 rounded-lg p-6">
                    ...
                    @php
                        ...
                        if ($originalUrl) {
                            if (preg_match('/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i', $originalUrl, $matches)) {
                                $videoId = $matches[1];
                                $isYouTubeVideo = true;
                            }
                        }
                
                        if (\Illuminate\Support\Str::endsWith(\Illuminate\Support\Str::lower($originalUrl), '.pdf')) {
                            $isPdf = true;
                        }
                    @endphp
                    ...
                    @if ($isYouTubeVideo || $isPdf)
                        <button onclick="document.getElementById('konten-media').classList.remove('hidden'); this.classList.add('hidden');"
                    ...
                    <div id="konten-media" class="hidden mb-6">
                        ...
                        <div class="relative mb-4">
                            <button 
                                onclick="openFullscreen('pdf-viewer')" 
                                class="absolute top-2 right-2 z-10 bg-purple-600 hover:bg-purple-700 text-white px-3 py-1 rounded text-sm">
                                Fullscreen
                            </button>
                    
                            <iframe 
                                id="pdf-viewer"
                                src="{{ asset($point->content_url) }}#toolbar=0&navpanes=0&scrollbar=0" 
                        ...
                        @else
                            <p class="text-gray-400">Konten tidak dikenali atau tidak didukung (bukan video YouTube atau PDF).</p>
                        @endif
                    </div>
                    ...
                    @if(!in_array($point->id, $completedPointIds))
                        <button id="markCompleteBtn" data-module-id="{{ $module->id }}" data-point-id="{{ $point->id }}"
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.space-y-2 a').forEach(link => {
                ...
                if (linkPathname === currentPathname || (linkPathname === '/' && currentPathname === '/dashboard')) {
            ...
            if (markCompleteBtn) {
                ...
                markCompleteBtn.addEventListener('click', function() {
                    ...
                    fetch(url, {
                        ...
                        body: JSON.stringify({}) // Anda mungkin perlu mengirim data tambahan di sini
                    })
                    .then(response => {
                ...
                console.warn('Tombol Mark as Complete TIDAK Ditemukan di DOM. Mungkin karena kondisi Blade atau ID salah.');
            }
        });
    </script>
@endpush
